...
for (k, solver) in enumerate(meths)
    ...
    tvec = (0:length(balance)-1) .* dt

    # Añadir filas al dataframe
    append!(data, DataFrame(
        t = tvec,
        method = fill(string(solver), length(balance)),
        value = balance
    ))
end


m1 = data[data.method .== "explicit_euler", :]
m2 = data[data.method .== "implicit_euler", :]
m3 = data[data.method .== "midpoint", :]
m4 = data[data.method .== "rk4", :]
m5 = data[data.method .== "exact", :]

boxplot(m1.value, label="explicit_euler", title="dt = 22.22222 x 10e6", legend=:right)
boxplot!(m2.value, label="implicit_euler")
boxplot!(m3.value, label="midpoint")
boxplot!(m4.value, label="rk4")
boxplot!(m5.value, label="exact")